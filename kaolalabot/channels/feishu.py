...
class FeishuChannel(BaseChannel):
    ...
    async def start(self) -> None:
        ...
        async with self._start_guard:
            if self._running:
                logger.warning("Feishu channel already running, ignore duplicated start()")
                return
        ...
        def run_ws() -> None:
            # Recreate ws client for every reconnect to avoid SDK internal event-loop reuse issues.
            # lark_oapi keeps a module-level `loop`; we must override it with this thread-local loop.
            ...
            while self._running and not self._stop_event.is_set():
                try:
                    ...
                    if "event loop is already running" in str(e).lower():
                        logger.warning("Detected loop re-entry from SDK, forcing fresh ws client on next retry")
                finally:
    ...
    async def _download_and_save_media(
        ...
        """
        loop = asyncio.get_running_loop()
        media_dir = Path.home() / ".kaolalabot" / "media"
    ...
    @staticmethod
    def _sanitize_outbound_content(text: str | None) -> str:
        """Filter internal metadata/code-like content before sending to Feishu users."""
        if not text:
            return ""
    ...
    async def send(self, msg: OutboundMessage) -> None:
        ...
        try:
            ...
            if msg.content and msg.content.strip():
                safe_content = self._sanitize_outbound_content(msg.content)
                if not safe_content:
                    safe_content = "\u4efb\u52a1\u5df2\u6267\u884c\u5b8c\u6210\u3002"
                card = {"config": {"wide_screen_mode": True}, "elements": self._build_card_elements(safe_content)}
                await loop.run_in_executor(
                    None, self._send_message_sync,
