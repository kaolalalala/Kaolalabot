"""Utility functions for kaolalabot."""
...
def get_data_path() -> Path:
    """~/.kaolalabot data directory."""
    return ensure_dir(Path.home() / ".kaolalabot")
...
def get_workspace_path(workspace: str | None = None) -> Path:
    ...
    # 回退到用户目录
    return ensure_dir(Path.home() / ".kaolalabot" / "workspace")
...
def sync_workspace_templates(workspace: Path, silent: bool = False) -> list[str]:
    ...
    try:
        tpl = pkg_files("kaolalabot") / "templates"
    except Exception:
        return []
    if not tpl.is_dir():
        return []
